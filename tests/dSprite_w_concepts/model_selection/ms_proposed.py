...
def tune_kernel(source_train,
  ...
  params = product(np.logspace(-4, 0, n_params).tolist(),
                   np.logspace(-4, 0, n_params).tolist())
  
  best_alpha, best_scale = None, None
  for alpha, scale in params:
    ...
    best_err_i = np.inf
    best_model_i = None
    best_alpha_i, best_scale_i = None, None
    for i, (train_idx, test_idx) in enumerate(kf.split(source_train['X'])):
      print(f"({alpha}, {scale}), Fold {i}:")
      ...
      estimator_full = FullAdapt(source_train_cv_train,
                                 target_train,
                                 source_test,
                                 target_test,
                                 split,
      ...
      err = mean_squared_error(source_train_cv_val['Y'],
                               estimator_full.predict({'X': source_train_cv_val['X']},
                                                       'source',
                                                       'source'))
      errs.append(err)

      if err < best_err_i:
        best_alpha_i, best_scale_i = alpha, scale 
        best_err_i = err
        best_model_i = estimator_full

    if np.mean(errs) < best_err:
      best_err = np.mean(errs)
      best_estimator = best_model_i
      best_alpha, best_scale = best_alpha_i, best_scale_i
    print(f"\nalpha: {best_alpha}, scale: {best_scale}, err: {np.mean(errs)}\n")
