...
from KPLA.models.plain_kernel.model_selection import tune_adapt_model
...
def load_data(s_path, w_id, seed, qu=1):
  source_df = pd.read_csv(f"{s_path}/synthetic_multivariate_num_samples_10000_w_coeff_{w_id}_p_u_0_0.9_{seed}.csv")
  ...
  #load target data
  target_df = pd.read_csv(f"{s_path}/synthetic_multivariate_num_samples_10000_w_coeff_{w_id}_p_u_0_0.{qu}_{seed}.csv")
...
#load data
source_train, source_val, source_test, target_train, target_val, target_test = load_data(source_path, w_idx, seed)

#perform model selection
best_estimator, best_params = tune_adapt_model(source_train,
                                                target_train,
                                                source_test,
                                                target_test,
                                                source_val,
...
for seed in range(192, 202):
  for qu in range(9, 0, -1):
    source_train, source_val, source_test, target_train, target_val, target_test = load_data(source_path, w_idx, seed, qu)
    ...
    df.to_csv(f"{store_path}/kernel_result_w{w_idx}_seed_{seed}_qu{qu}.csv", sep=",",
              index=False,
              encoding="utf-8")
