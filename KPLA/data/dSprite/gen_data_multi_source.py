...
from KPLA.data.dSprite.gen_data_wpc import (
    U2imgs,
    img2X,
    XU2C,
    CU2Y,
...
def Z2U(Z, U_dists):
    U = np.apply_along_axis(
        lambda x: sample_beta(x, U_dists), axis=1, arr=Z
    ).reshape(-1, 1)
...
def U2W(U, pos_X_basis, pos_Y_basis):
    ...
    for i in tqdm(range(N), desc="getting U rotation matrix"):
        ...
        U_W[i][0] = (
            10 * (rot_mat @ np.array([[pos_X_basis], [pos_Y_basis]]))[1, 0]
        )  # pos_Y
...
def CU2Y_v2(C, U, pos_X_basis, pos_Y_basis, task="regression"):
    var = np.random.normal(0, 0.1, size=U.shape)
    ...
    for i in tqdm(range(N), desc="getting U rotation matrix"):
        rot_mat = get_rot_mat(U[i][0])
        U_Y[i][0] = (rot_mat @ np.array([[pos_X_basis], [pos_Y_basis]]))[
            1, 0
        ]  # pos_Y
...
def generate_samples_Z2U(
    ...
    if target:
        U = 2.0 * np.pi * np.random.uniform(alpha_2, 1, size=(Z.shape[0], 1))
    else:
        U = Z2U(Z, U_dists)

    imgs_sampled = U2imgs(
        U, metadata, pos_X_basis_idx, pos_Y_basis_idx, imgs, imgs_basis
    )
    ...
    if not target:
        Z = (
            OneHotEncoder(categories=[list(range(N_ENVS))])
            .fit_transform(Z)
            .toarray()
        )
