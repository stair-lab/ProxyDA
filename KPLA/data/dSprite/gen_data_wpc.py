...
def sample_latent(metadata, size=1, p_dict=None):
    ...
    for lat_i, lat_size in enumerate(latents_sizes):
        if p_dict is None:
            ...
            samples[:, lat_i] = np.random.choice(
                range(lat_size), p=p_dict[lat_i], size=size
            )
...
def U2imgs(U, metadata, pos_X_basis_idx, pos_Y_basis_idx, imgs, imgs_basis):
    ...
    for i in tqdm(range(N), desc="applying U image rotation"):
        imgs_sampled[i] = crop_center(
            rotate(imgs_basis[latents[i][1]], np.rad2deg(U_[i][0])), dx, dy
        )
...
def img2X(img_samples):
    ...
    return img_samples + np.random.multivariate_normal(
        np.zeros(d), 0.001 * np.eye(d), size=(N, d)
    )
...
def U2W(U, pos_X_basis, pos_Y_basis):
    ...
    for i in tqdm(range(N), desc="getting U rotation matrix"):
        rot_mat = get_rot_mat(U[i][0])
        U_W[i][0] = (
            10 * (rot_mat @ np.array([[pos_X_basis], [pos_Y_basis]]))[0, 0]
        )  # pos_X
...
def generate_samples(
    ...
    val = {"X": X_val, "C": C_val, "Y": Y_val, "W": W_val, "U": U_val}
    test = {"X": X_test, "C": C_test, "Y": Y_test, "W": W_test, "U": U_test}
