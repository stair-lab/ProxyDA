...
class dfaDataSetTorch(NamedTuple):
    ...
    def from_numpy(cls, train_data: dfaDataSet):
        ...
        return dfaDataSetTorch(
            C=torch.tensor(np.asarray(train_data.C), dtype=torch.float32),
    ...
    def to_gpu(self):
        return dfaDataSetTorch(
            C=self.C.cuda(), X=self.X.cuda(), W=self.W.cuda(), Y=self.Y.cuda()
        )
...
def split_train_data(train_data: dfaDataSetTorch, n_split):
    ...
    for s_idx in split_index:
        ...
        batch_data = dfaDataSetTorch(
            *[get_data(data, idx) for data in train_data]
        )
...
class mdfaDataSetTorch(NamedTuple):
    ...
    def from_numpy(cls, train_data: mdfaDataSet):
        ...
        return mdfaDataSetTorch(
            E=torch.tensor(np.asarray(train_data.E), dtype=torch.float32),
            X=torch.tensor(np.asarray(train_data.X), dtype=torch.float32),
            W=torch.tensor(np.asarray(train_data.W), dtype=torch.float32),
            Y=torch.tensor(np.asarray(train_data.Y), dtype=torch.float32),
        )
    ...
    def to_gpu(self):
        return mdfaDataSetTorch(
            E=self.E.cuda(), X=self.X.cuda(), W=self.W.cuda(), Y=self.Y.cuda()
        )
