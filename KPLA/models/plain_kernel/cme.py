...
class ConditionalMeanEmbed:
    ...
    def __init__(
        ...
        Args:
            ...
            lam_min: minimum of lambda (log space) for hyperparameter tuning,
                float
            lam_min: maximum of lambda (log space) for hyperparameter tuning,
                float
            kernel_dict: Dictionary of kernel_function, dictionary keys are the
                variable name
        ...
        if self.method == "original":
            gx = self.ker_xx + self.lam * self.n_samples * jnp.eye(
                self.n_samples
            )
