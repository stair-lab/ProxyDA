...
class BridgeH0:
    ...
    def __init__(
        ...
        ker_cc = ker_mat(
            jnp.array(c), jnp.array(c), kernel=kernel_dict["C"], scale=self.sc
        )
        self.c = c
    ...
    def model_select(self, n_sample, ker_ww, ker_cc, gamma_xc, y):
        ...
        if (n_sample >= 1000) or (ker_ww.shape[0] > 1000):
            select_id = np.random.choice(
                n_sample, min(n_sample, 1000), replace=False
            )
            select_id2 = np.random.choice(
                ker_ww.shape[0], min(ker_ww.shape[0], 1000), replace=False
            )
        ...
        sub_sigma = hadamard_prod(
            mat_mul(mat_mul(gamma_sub_xc.T, ker_sub_ww), gamma_sub_xc),
            ker_sub_cc,
        )
