...
class MultiKernelMethod(KernelMethod):
    ...
    def fit(self, train_target=True, task='r'):
        ...
        if self.split:
            ...
            for i in range(n_env):
                cat_source_train = concatenate_data(self.source_train[i], cat_source_train)
            self.source_train = [cat_source_train, self.source_train] 
            #first train cme(w|c,x), h0 with data pulled across environments, then the second item train the cme(w|x) sepcifically for each environment 
            self.target_train = [self.target_train[0], self.target_train]
            #first train cme(w|c,x), h0 with data, then the second item train the cme(w|x) sepcifically for each environment 
    ...
    def evaluation(self, task='r'):
        ...
        for i in range(n_env):
            ...
            eval_list.append(flatten({'task': 'source-source', 'env_id': int(i), 'predict error': ss_error}))
        

            # source on target error
            predictY = self.predict(target_testX, 'source', 'source', i)
            st_error = self.score(predictY,  target_testY, task)
            eval_list.append(flatten({'task': 'source-target', 'env_id': int(i), 'predict error': st_error}))
            """
            #target on source
            predictY = self.predict(source_testX, 'target', 'target', 0)
            ts_error = self.score(predictY,  source_testY)
            eval_list.append({'task': 'target-source', 'env_id': i, 'predict error': ts_error})
            """        
 
        # target on target errror
        predictY = self.predict(target_testX, 'target', 'target', 0)
        tt_error = self.score(predictY, target_testY, task)
        eval_list.append(flatten({'task': 'target-target', 'env_id': 0+n_env, 'predict error': tt_error}))


        #adaptation error
        predictY = self.predict(target_testX, 'source', 'target', 0)
        adapt_error = self.score(predictY,  target_testY, task)
        eval_list.append(flatten({'task': 'adaptation', 'env_id': 0+n_env, 'predict error': adapt_error}))
            ...
            if i == 0:
                ...
                for j, idx in enumerate(split_id):
                    if j < 2:
                        ...
                        train_list[2].append(split_data_widx(self.source_train[i], idx))
        print('concatenate data across source domains')
        print('number of training samples for cme(w|z,x):', train_list[0]['X'].shape[0])
        print('number of training samples for h_0:', train_list[1]['X'].shape[0])
        print('number of training samples for cme(w|x):', [d['X'].shape[0] for d in train_list[2]])
