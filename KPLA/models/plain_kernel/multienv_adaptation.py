...
from KPLA.models.plain_kernel.bridge_m0 import (
    BridgeM0,
    BridgeM0CAT,
    BridgeM0CLF,
    BridgeM0CATCLF,
...
class MultiEnvAdapt(MultiKernelMethod):
    ...
    def _fit_one_domain(self, domain_data, task="r"):
        ...
        cme_w_xz = ConditionalMeanEmbed(
            jnp.array(train_data["W"]),
            covars,
            self.lam_set["cme"],
...
class MultiEnvAdaptCAT(MultiKernelMethod):
    ...
    def _fit_one_domain(self, domain_data, task="r"):
        ...
        unique_z, indices = jnp.unique(
            jnp.array(train_data["Z"]), return_inverse=True
        )
        unique_z = np.asarray(unique_z)
        cme_w_xz_lookup = {}
        ...
        for i, z in enumerate(unique_z):
            ...
            cme_w_xz = ConditionalMeanEmbed(
                jnp.array(train_data["W"][select_id, ...]),
                covars,
                self.lam_set["cme"],
                kernel_dict=self.kernel_dict["cme_w_xz"],
                scale=self.sc,
