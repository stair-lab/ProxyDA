...
def modif_kron(x, y):
    if y.shape[1] != x.shape[1]:
        ...
        return jnp.array(
            [jnp.kron(x[:, i], y[:, i]).T for i in range(y.shape[1])]
        )
...
def ker_mat(x1, x2, kernel="rbf", scale=1.0):
    """
    Compute the K_xx
...
def woodbury_identity(q_mat, lam, n):
    ...
    q = q_mat.shape[1]
    inv_temp = jsla.solve(
        lam * n * jnp.eye(q) + mat_mul(q_mat.T, q_mat), jnp.eye(q)
    )
    if jnp.isnan(inv_temp).any():
        print("inv_temp is nan")
    aprox_k = (jnp.eye(n) - mat_mul(mat_mul(q_mat, inv_temp), q_mat.T)) / (
        lam * n
    )
    return aprox_k
