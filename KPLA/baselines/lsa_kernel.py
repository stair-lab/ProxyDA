...
from latent_shift_adaptation.methods.continuous_spectral_method.utils import (
    compute_adaggerb_multi,
    least_squares,
    multi_least_squares_scale,
)
from latent_shift_adaptation.methods.continuous_spectral_method.create_basis import (
    basis_from_centers,
)
from latent_shift_adaptation.methods.continuous_spectral_method.multi_ls_conditional_de import (
    MultiCDEBase,
)
from latent_shift_adaptation.methods.continuous_spectral_method.multi_ls_de import (
    MultiDEBase,
)
from latent_shift_adaptation.methods.continuous_spectral_method.multi_ls_marginal_de import (
    MultiMDEBase,
)
...
def train_process(
    ...
    basisx = [
        basis_from_centers(params["mu_x_u_mat"].squeeze()[i], 1)
        for i in range(params["k_x"])
    ]
    ...
    cy_id = np.where(
        (np.sum(source_data_sample["c"] == test_c, axis=1) == 3)
        & (source_data_sample["y"] == test_y)
    )[0]
    ...
    # f(x,w,c=test_c)
    fxw_c = MultiDEBase(
        [sx_c[:, 0][:, np.newaxis], sx_c[:, 1][:, np.newaxis], sw_c],
        basis,
        1e-2,
    )
    # f(x,w,c=test_c,y=test_y)
    fxwy0_c = MultiDEBase(
        [sx_cy0[:, 0][:, np.newaxis], sx_cy0[:, 1][:, np.newaxis], sw_cy0],
        basis,
        1e-2,
    )
    ...
    D, _, y_coor = compute_adaggerb_multi(fxw_c_df, fxwy0_c_df)
    ...
    for func in eigen_func:
        ...
        for b in baselist:
            ...
            vec.append(
                np.sqrt(2 * np.pi) * np.sum(b.get_params()["weight"]) * l
            )
    ...
    for j in range(len(fw_u)):
        ...
        plt.plot(
            new_w,
            p_w,
            color=colors((j + 2) * 4 + 0),
            label="fw_u %d cdf: %.2f" % (j + 1, l1_sum),
        )
    plt.legend(bbox_to_anchor=(1.1, 0.6))
    plt.plot(
        new_w,
        stats.norm.pdf(new_w, params["mu_w_u_mat"][0], 1),
        "-.",
        color="b",
        label="true density function 1",
    )
    plt.plot(
        new_w,
        stats.norm.pdf(new_w, params["mu_w_u_mat"][1], 1),
        "-.",
        color="r",
        label="true density function 2",
    )
        ...
        for i, x in enumerate(new_x):
            for k, x2 in enumerate(new_x):
                for j, w in enumerate(new_w):
                    ...
                    true_pdf[i, k, j] = multi_true_p_w_x(
                        w, n_x, p_u_source, params
                    )
    ...
    if evaluate:
        ...
        for i, x in enumerate(new_x):
            for j, y in enumerate(new_x):
                n_x = np.array([x, y])
                cosde_pdf[i, j] = fx.get_pdf(
                    [x.reshape(1, 1), y.reshape(1, 1)]
                )
                true_pdf[i, j] = multi_true_p_x(
                    n_x,
                    p_u_source,
                    params["mu_x_u_mat"].squeeze() * params["mu_x_u_coeff"],
                )
    ...
    if method == "kmeans":
        ...
        kmeans = KMeans(n_clusters=2, random_state=0).fit(
            np.array(source_data_sample["x"]).reshape(-1, 2)
        )
        ...
        random_id = np.random.choice(
            source_data_sample["x"].shape[0], 100, replace=False
        )
    ...
    if evaluate:
        ...
        print(
            "MSE of q(U)/p(U):",
            np.mean(
                (qu_pu - np.array(p_u_target) / np.array(p_u_source)) ** 2
            ),
        )
        ...
        for i, x in enumerate(select_x):
            pU_x_mat[i] = compute_pu_x(fw_u, fw_x, x)
            pU_x_mat_true[i] = np.array(
                multi_true_p_u_x(
                    x,
                    p_u_source,
                    params["mu_x_u_coeff"] * params["mu_x_u_mat"].squeeze(),
                )
            ).squeeze()
    ...
    # Learn p(y|x) via MLP
    mlp_p_y_x = MLPClassifier(
        random_state=0, learning_rate="adaptive", max_iter=10000
    ).fit(np.array(source_data_sample["x"]), np.array(source_data_sample["y"]))
    ...
    if evaluate:
        ...
        for x in source_data_sample["x"]:
            mse.append(
                (
                    multi_true_p_y_x(x, p_u_source, params)
                    - mlp_p_y_x.predict_proba(x.reshape(-1, 2))[:, 1]
                )
                ** 2
            )
        ...
        for i, x1 in enumerate(new_x):
            for j, x2 in enumerate(new_x):
                for k, w in enumerate(new_w):
                    ...
                    fwy1_x0_pdf[i, j, k] = (
                        fw_y1x0.eval(w.reshape(1, 1))
                        * mlp_p_y_x.predict_proba(n_x.reshape(1, 2))[0, 1]
                    )
                    true_fwy1_x0_pdf[i, j, k] = multi_true_p_yw_x(
                        1, w, n_x, p_u_source, params
                    )

                    fwy0_x0_pdf[i, j, k] = (
                        fw_y0x0.eval(w.reshape(1, 1))
                        * mlp_p_y_x.predict_proba(n_x.reshape(1, 2))[0, 0]
                    )
                    true_fwy0_x0_pdf[i, j, k] = multi_true_p_yw_x(
                        0, w, n_x, p_u_source, params
                    )

        print(
            "MSE of p(Y=1,w|x):",
            np.mean((fwy1_x0_pdf - true_fwy1_x0_pdf) ** 2),
        )
        print(
            "MSE of p(Y=0,w|x):",
            np.mean((fwy0_x0_pdf - true_fwy0_x0_pdf) ** 2),
        )
...
def inference(
    dataset, qu_pu, fw_y0x, fw_y1x, p_y_x, fw_u, fw_x, true_p_u, params
):
    ...
    source_mse = []
    error = 0
    for (x,) in source_feature:
        # Predicting the probability that q(Y=0|x)
        qy_x, yxu_err = predict(
            qu_pu, fw_y0x, fw_y1x, p_y_x, fw_u, fw_x, x, params, "standard"
        )
        qy_x = qy_x[1]
...
def predict(
    qu_pu, fw_y0x, fw_y1x, py_x, fw_u, fw_x, x0, params, normalize="standard"
):
    ...
    fwy0_x0 = LSEigenBase(
        fw_y0x0.get_params()["base_list"],
        fw_y0x0_coeff * py_x.predict_proba(x0.reshape(1, params["k_x"]))[0, 0],
    )

    fw_y1x0_coeff = fw_y1x0.get_params()["coeff"]
    fwy1_x0 = LSEigenBase(
        fw_y1x0.get_params()["base_list"],
        fw_y1x0_coeff * py_x.predict_proba(x0.reshape(1, params["k_x"]))[0, 1],
    )

    solution = multi_least_squares_scale(
        fwu_x0, fwy0_x0, fwy1_x0, pU_x0, reuse_gram=False
    )
    fy0_x0u = solution[0:2]
    fy1_x0u = solution[2::]
    sum_u0 = fy0_x0u[0] + fy1_x0u[0]
    sum_u1 = fy0_x0u[1] + fy1_x0u[1]
    sum_u = np.array([1.0 / sum_u0, 1.0 / sum_u1])
    fy0_x0u = fy0_x0u * sum_u
    fy1_x0u = fy1_x0u * sum_u

    true_fy0_x0u = np.array(
        [
            1 - multi_true_p_y_ux(x0, 0, params),
            1 - multi_true_p_y_ux(x0, 1, params),
        ]
    ).squeeze()

    true_fy1_x0u = np.array(
        [multi_true_p_y_ux(x0, 0, params), multi_true_p_y_ux(x0, 1, params)]
    ).squeeze()
    mse_fy_xu = np.mean(
        np.array([fy0_x0u - true_fy0_x0u, fy1_x0u - true_fy1_x0u]) ** 2
    )
    qy0_x0 = max(0.0, sum(fy0_x0u * qu_pu * pU_x0))
    qy1_x0 = max(0.0, sum(fy1_x0u * qu_pu * pU_x0))
...
def inference_with_qux(
    dataset, fw_y0x, fw_y1x, p_y_x, fw_u, fw_x, gw_x, true_p_u, params
):
    source_feature = np.array(dataset["x"])
    source_label = np.array(dataset["y"])
    ...
    for x, _ in zip(source_feature, source_label):
        # Predicting the probability that q(Y=0|x)
        qy_x, yxu_err = predict_with_qux(
            fw_y0x, fw_y1x, p_y_x, fw_u, fw_x, gw_x, x, params, "standard"
        )
...
def predict_with_qux(
    fw_y0x, fw_y1x, py_x, fw_u, fw_x, gw_x, x0, params, normalize="standard"
):
    """Given fixed y, estimate f(y|x0, U=i) for i=1,...,k
    ...
    qy0_x0 = max(0.0, sum(fy0_x0u * qU_x0))
    qy1_x0 = max(0.0, sum(fy1_x0u * qU_x0))
    out_prob = np.array([qy0_x0, qy1_x0])
    if normalize == "standard":
        out_prob /= np.sum(out_prob)
    else:
