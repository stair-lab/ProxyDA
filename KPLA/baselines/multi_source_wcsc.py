...
class MultiSourceMMD:
    ...
    def __init__(self, source_data, kernel, kde_kernel, bandwidth=1.0):
        ...
        a_mat = np.zeros(
            (self.n_env * self.n_label, self.n_env * self.n_label)
        )
        ...
        def element_a(i, j, k, m):
            ...
            if self.count_table[i, k] == 0.0:
                self.count_table[i, k] = n_ik
            if self.count_table[j, m] == 0.0:
            ...
            for j in range(self.n_env):
                for k in range(self.n_label):
                    for m in range(self.n_label):
                        a_mat[i * self.n_label + k, j * self.n_label + m] = (
                            element_a(i, j, k, m)
                        )
    ...
    def _get_kde_x_y(self, k):
        ...
        for i in range(self.n_env):
            ...
            kde = KernelDensity(
                bandwidth=self.bandwidth, kernel=self.kde_kernel
            ).fit(xi_k)
    ...
    def _get_beta(self, b):
        ...
        d = matrix(np.ones(1))
        g_mat = matrix(-np.eye(self.n_env * self.n_label))
        h = matrix(np.zeros(self.n_env * self.n_label))
...
class MuiltiSourceCombCLF(MultiSourceMMD):
    ...
    def predict(self, x):
        ...
        for j in range(self.n_label):
            tmp = (
                self._get_pdf_kde_x_y(j, x) * self.alpha_[:, j][:, np.newaxis]
            )
