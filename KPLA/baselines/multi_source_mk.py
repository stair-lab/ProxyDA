...
class MultiSourceMK:
    ...
    def fit(self, source_data, target_data):
        ...
        for j, source_d in enumerate(source_data):
            dist_weight[j] = self._compute_pdist(
                np.array(source_d["X"]), np.array(target_data["X"])
            )
        ...
        for i in range(self.n_env):
            ...
            for j in range(i, self.n_env):
                w = self._compute_pdist(
                    np.array(source_data[i]["X"]),
                ...
                ker_xx = self.x_kernel(
                    np.array(source_data[i]["X"]),
                    np.array(source_data[j]["X"]),
                )

                big_feature[
                    start_x : start_x + len_i, start_y : start_y + len_j
                ] = (ker_xx * w)
                big_feature[
                    start_y : start_y + len_j, start_x : start_x + len_i
                ] = (ker_xx.T) * w
